...
assert_eq "  ...and exits 1"                           "$RC" "1"
assert_eq "show prints another Mac's file"            "$(renv "${H2[@]}" -- claude show studio CLAUDE.md)" "# studio rules"
assert_eq "  ...and this Mac's"                        "$(renv "${H2[@]}" -- claude show this CLAUDE.md)" "# laptop rules"
assert_contains "  ...files only"                      "$(renv "${H2[@]}" -- claude show studio ../.claude.json 2>&1)" "CLAUDE.md or scripts/<name>"
assert_contains "  ...says when it is not there"       "$(renv "${H2[@]}" -- claude show studio scripts/nope.sh 2>&1)" "no scripts/nope.sh on studio"
